...
// Daftar Tahapan Workflow Utama (Urutan Estafet Maju)
const STAGE_NAMES = [
  '1. Concept',
  '2. Copywriting',
  '3. Produksi / Syuting',
...
export default function WorkflowWorkspace() {
  ...
  // State Akun Login (Default Admin/Planner)
  const [currentAccount, setCurrentAccount] = useState(ACCOUNTS[0]);
  const [isLoginModalOpen, setIsLoginModalOpen] = useState(false);

  // State untuk Mobile Sidebar Drawer
  ...
  const loadData = async (targetSheet?: string) => {
    try {
      ...
      if (json.success && json.sheets) {
        setData(json);
        if (targetSheet && json.sheetNames?.includes(targetSheet)) {
          setSelectedSheet(targetSheet);
        } else if (!selectedSheet || !json.sheetNames?.includes(selectedSheet)) {
          const defaultSheet = json.sheetNames?.find((n: string) => n.includes('26')) || json.sheetNames?.[0];
  ...
  const contentList = rows.map((r: any[]) => {
    ...
    return {
      id: r[0] || '-',
      week: r[1] || 'Week 1',
      date: r[2] ? String(r[2]).split('T')[0] : '-',
      title: r[5] ? String(r[5]).trim() : '',
      stage: parsedStage,
  ...
  const sortedSheetNames = MONTH_ORDER.filter(m => data?.sheetNames?.includes(m));
    ...
    <div style={styles.container}>
      ...
      <div className="mobile-header-bar" style={styles.mobileHeaderBar}>
        ...
        <div 
          ...
          style={{ fontSize: '11px', fontWeight: 600, color: '#10B981', backgroundColor: '#ECFDF5', padding: '4px 10px', borderRadius: '8px', border: '1px solid #A7F3D0' }}
        >
      ...
      {/* SIDEBAR DRAWER (Mobile Popup Menu) */}
      {isMobileSidebarOpen && (
        ...
        <header className="main-header" style={styles.header}>
          ...
          <div className="header-actions-row" style={styles.headerActions}>
            ...
            <input
              ...
              type="text"
              placeholder="Cari judul atau tim..."
              value={searchQuery}
          ...
          <div className="grid-container-cards" style={styles.gridContainer}>
            {contentList.length === 0 ? (
              <div style={styles.emptyState}>
                ...
                <span style={{ fontSize: '11px', color: '#A1A1AA', marginTop: '6px', display: 'inline-block' }}>
                  Gunakan akun Admin / Planner untuk mulai input konten baru.
                </span>
              </div>
            ) : (
                ...
                return (
                  <div key={idx} style={styles.card}>
                    ...
                    <div style={{
                      ...
                      borderColor: isFinished ? '#86EFAC' : '#FDE68A',
                      color: isFinished ? '#166534' : '#92400E'
                    }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <span style={{ fontSize: '12px', fontWeight: 700 }}>{item.stage}</span>
                        <span style={{ fontSize: '10px', fontWeight: 800, padding: '2px 8px', borderRadius: '6px', backgroundColor: isFinished ? '#166534' : '#D97706', color: '#FFF', letterSpacing: '0.05em' }}>
              ...
              {ACCOUNTS.map((acc, i) => (
                <button
                  ...
                  style={{
                    padding: '12px 16px',
                    borderRadius: '12px',
                    border: currentAccount.name === acc.name ? '2px solid #09090B' : '1px solid #E4E4E7',
                    backgroundColor: currentAccount.name === acc.name ? '#F4F4F5' : '#FFFFFF',
                    textAlign: 'left',
                    cursor: 'pointer',
                    fontWeight: 600,
                    fontSize: '13px',
        ...
        <div style={styles.modalOverlay}>
          <div className="modal-box-card" style={styles.modalCard}>
            <div style={styles.modalHeader}>
              <div>
                ...
                <p style={styles.modalSub}>
                  {isEditing ? `Sedang dikerjakan oleh: ${currentAccount.name}` : 'Konten baru otomatis dimulai dari 1. Concept.'}
                </p>
              </div>
            ...
            <form onSubmit={handleAddOrUpdateContent} style={styles.form}>
              ...
              {/* 2 PILIHAN CHECKLIST: PROSES / SELESAI */}
              <div>
                <label style={styles.label}>Status Pengerjaan</label>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginTop: '4px' }}>
                  <button
                    type="button"
                    onClick={() => setStatusCheck('Proses')}
                    style={{
                      padding: '10px',
                      borderRadius: '10px',
                      border: statusCheck === 'Proses' ? '2px solid #D97706' : '1px solid #E4E4E7',
                      backgroundColor: statusCheck === 'Proses' ? '#FEF3C7' : '#FAFAFA',
                      fontWeight: 700,
                      fontSize: '12px',
                      color: '#92400E',
                      cursor: 'pointer'
                  ...
                  <button
                    type="button"
                    onClick={() => setStatusCheck('Selesai')}
                    style={{
                      padding: '10px',
                      borderRadius: '10px',
                      border: statusCheck === 'Selesai' ? '2px solid #166534' : '1px solid #E4E4E7',
                      backgroundColor: statusCheck === 'Selesai' ? '#DCFCE7' : '#FAFAFA',
                      fontWeight: 700,
                      fontSize: '12px',
                      color: '#166534',
                      cursor: 'pointer'
                ...
                <span style={{ fontSize: '11px', color: '#71717A', marginTop: '6px', display: 'block', fontWeight: 500 }}>
                  Jika memilih <b>Selesai</b>, sistem akan otomatis mengoper/memindahkan konten ke tahap jobdesk berikutnya.
                </span>
              </div>
              ...
              <div style={styles.modalFooter}>
                ...
                <button type="submit" style={styles.primaryButton}>
                  {isEditing ? 'Simpan Estafet' : 'Simpan Konten Baru'}
                </button>
              </div>
            </form>
          </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  accountCard: { backgroundColor: '#FAFAFA', border: '1px solid #E4E4E7', padding: '10px 12px', borderRadius: '10px', cursor: 'pointer', transition: 'background 0.2s' },
  ...
  header: { padding: '20px 32px', backgroundColor: '#FFFFFF', borderBottom: '1px solid #E4E4E7', display: 'flex', justifyContent: 'space-between', alignItems: 'center', zIndex: 20, marginTop: '0px' },
